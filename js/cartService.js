...
function agregarAlCarrito(producto){
  ...
  let memoria = JSON.parse(localStorage.getItem("data_bookshops"));
  let cantidadProductoFinal;
  //Si no hay localstorage lo creo
  if(!memoria || memoria.length === 0) {
    const nuevoProducto = getNuevoProductoParaMemoria(producto)
    localStorage.setItem("data_bookshops",JSON.stringify([nuevoProducto]));
    actualizarNumeroCarrito();
    cantidadProductoFinal = 1;
  }
  else {
    // Si hay localstorage me fijo si el artículo ya está ahí
    const indiceProducto = memoria.findIndex(data_bookshop => data_bookshop.id === producto.id)
    ...
    localStorage.setItem("data_bookshops",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("data_bookshops"));
  if(memoria && memoria.length > 0){
...
function reiniciarCarrito(){
  localStorage.removeItem("data_bookshops");
  actualizarNumeroCarrito();
}
